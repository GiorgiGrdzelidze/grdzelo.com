        ...
        <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            <div class="mx-auto max-w-2xl text-center">
                ...
                <p class="mt-4 text-lg text-muted-foreground">
                    Academic background and qualifications that shape my expertise.
                </p>
            </div>
                ...
                >
                    <div class="flex items-start gap-4">
                        <div v-if="edu.logo" class="hidden shrink-0 sm:block">
                            <img :src="`/storage/${edu.logo}`" :alt="edu.institution" class="h-12 w-12 rounded-lg object-contain border border-border/40 p-1" />
                        </div>
                        <div class="flex-1">
                            <div class="flex items-center gap-2">
                                <h2 class="text-lg font-semibold">{{ edu.degree || 'Studies' }}</h2>
                                <Badge v-if="edu.is_current" variant="default" class="text-xs">Current</Badge>
                                <Badge v-if="edu.is_featured" variant="secondary" class="text-xs">Featured</Badge>
                            </div>
                            <p v-if="edu.field_of_study" class="mt-0.5 text-sm font-medium text-muted-foreground">
                                {{ edu.field_of_study }}
                            </p>
                            ...
                            <p v-if="edu.description" class="mt-3 text-sm text-muted-foreground leading-relaxed">{{ edu.description }}</p>
                            <ul v-if="edu.achievements?.length" class="mt-3 space-y-1">
                                <li v-for="(a, i) in edu.achievements" :key="i" class="flex items-start gap-2 text-sm text-muted-foreground">
